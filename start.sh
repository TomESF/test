...
ARGO_AUTH=${ARGO_AUTH:-''}
WSPATH=${WSPATH:-'argo'}
UUID=${UUID:-'113195ac-5c8a-4d38-8067-2f5057d70380'}
...
echo -e vless链接已经生成, icook.hk 可替换为CF优选IP'\n' > list.txt 
echo "vless://$UUID@icook.hk:443?encryption=none&security=tls&type=ws&host=$argo&path=$urlpath#$(echo $isp | sed -e 's/_/%20/g' -e 's/,/%2C/g')_tls" | tee -a list.txt sub.txt
echo -e '\n'端口 443 可改为 2053 2083 2087 2096 8443'\n' >> list.txt
echo "vless://$UUID@icook.hk:80?encryption=none&security=none&type=ws&host=$argo&path=$urlpath#$(echo $isp | sed -e 's/_/%20/g' -e 's/,/%2C/g')" | tee -a list.txt 
